...
namespace seeta
{
  class ImageData : public SeetaImageData {
    public:
      ...
      ImageData(int width, int height, int channels) :
        ...
        this->m_data.reset(
          new byte [this->count()], std::default_delete<byte []>());
  ...
  class Rect : public SeetaRect {
    public:
      ...
      Rect(int x, int y, int width, int height) :
        supper({ x, y, width, height }) {}

      Rect() : Rect(0, 0, 0, 0) {}

      Rect(int x, int y, const Size &size) :
        supper({ x, y, size.width, size.height }) {}

      Rect(const Point &top_left, int width, int height) :
        supper({ top_left.x, top_left.y, width, height }) {}

      Rect(const Point &top_left, const Size &size) :
        supper({ top_left.x, top_left.y, size.width, size.height }) {}
  ...
  class Region : public SeetaRegion {
    public:
      ...
      Region(int top, int bottom, int left, int right) :
        supper({ top, bottom, left, right }) {}

      Region() : Region(0, 0, 0, 0) {}

      Region(const Rect &rect) :
        Region(rect.y, rect.y + rect.height, rect.x, rect.x + rect.width) {}
  ...
  class ModelSetting : public SeetaModelSetting {
    public:
      ...
      ModelSetting(const std::string &model, SeetaDevice device) :
        self(model, device, 0) {}

      ModelSetting(const std::string &model, Device device, int id) :
        self(model, SeetaDevice(device), id) {}

      ModelSetting(const std::string &model, Device device) :
        self(model, SeetaDevice(device)) {}

      ModelSetting(const std::string &model) : self(model, SEETA_DEVICE_AUTO) {}

      ModelSetting(
        const std::vector<std::string> &model, SeetaDevice device, int id) :
        supper({ device, id, nullptr }) {
        this->append(model);
      }

      ModelSetting(const std::vector<std::string> &model, SeetaDevice device) :
        self(model, device, 0) {}

      ModelSetting(
        const std::vector<std::string> &model, Device device, int id) :
        self(model, SeetaDevice(device), id) {}

      ModelSetting(const std::vector<std::string> &model, Device device) :
        self(model, SeetaDevice(device)) {}

      ModelSetting(const std::vector<std::string> &model) :
        self(model, SEETA_DEVICE_AUTO) {}
      ...
      void append(const std::vector<std::string> &model) {
        this->m_model_string.insert(
          this->m_model_string.end(), model.begin(), model.end());
        this->update();
      }
      ...
      size_t count() const { return this->m_model_string.size(); }
    private:
  ...
  class Buffer : public SeetaBuffer {
    public:
      ...
      Buffer(self &&other) :
        supper({ other.buffer, other.size }),
      ...
      /**
       * \brief contruct cpp-style-buffer from c-style-buffer, optional borrow
       * or copy \param other c-style-buffer \param borrow if borrow or copy
       * buffer
       */
      ...
      Buffer(const void *buffer, int64_t size) : supper({ nullptr, 0 }) {
        ...
        if (this->m_size) {
          this->m_buffer.reset(
            new byte [size_t(this->m_size)], std::default_delete<byte []>());
        }
        this->buffer = this->m_buffer.get();
        this->size   = this->m_size;
        if (buffer != nullptr) { this->copy_from(buffer, size); }
      }
      ...
      Buffer clone() const { return self(this->buffer, this->size); }

      void rebind(const void *buffer, int64_t size) {
        if (size < 0) size = 0;
        if (size > this->m_size) {
          this->m_buffer.reset(
            new byte [size_t(size)], std::default_delete<byte []>());
        }
        this->m_size = size;
        this->buffer = this->m_buffer.get();
        this->size   = this->m_size;
  ...
  class ModelBuffer : SeetaModelBuffer {
    public:
      ...
      ModelBuffer(const seeta::Buffer &buffer, SeetaDevice device) :
        self(buffer, device, 0) {}

      ModelBuffer(const seeta::Buffer &buffer, Device device, int id) :
        self(buffer, SeetaDevice(device), id) {}

      ModelBuffer(const seeta::Buffer &buffer, Device device) :
        self(buffer, SeetaDevice(device)) {}

      ModelBuffer(const seeta::Buffer &buffer) :
        self(buffer, SEETA_DEVICE_AUTO) {}

      ModelBuffer(
        const std::vector<seeta::Buffer> &buffer, SeetaDevice device, int id) :
        supper({ device, id, nullptr }) {
        this->append(buffer);
      }

      ModelBuffer(
        const std::vector<seeta::Buffer> &buffer, SeetaDevice device) :
        self(buffer, device, 0) {}

      ModelBuffer(
        const std::vector<seeta::Buffer> &buffer, Device device, int id) :
        self(buffer, SeetaDevice(device), id) {}

      ModelBuffer(const std::vector<seeta::Buffer> &buffer, Device device) :
        self(buffer, SeetaDevice(device)) {}

      ModelBuffer(const std::vector<seeta::Buffer> &buffer) :
        self(buffer, SEETA_DEVICE_AUTO) {}
      ...
      void append(const std::vector<seeta::Buffer> &model) {
        this->m_model_buffer.insert(
          this->m_model_buffer.end(), model.begin(), model.end());
        this->update();
      }
      ...
      size_t count() const { return this->m_model_buffer.size(); }
    private:
      ...
      void update() {
        ...
        this->m_buffer.push_back(
          { nullptr, 0 });  // terminate with empty buffer
