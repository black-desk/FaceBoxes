  ...
  namespace v6
  {
    ...
    static orz::jug read_jug_from_json_or_sta(const std::string &filename) {
      ...
      try {
        ...
        ORZ_LOG(orz::ERROR)
          << "Model must be sta or json file, given: " << filename
          << orz::crash;
        return orz::jug();
      }
    }

    static Module parse_tsm_module(
      const orz::jug &model, const std::string &root) {
      if (model.valid(orz::Piece::BINARY)) {
        ...
        if (
          commands.size() != 3 || ! commands [0].empty()
          || commands [1] != "file") {
          ORZ_LOG(orz::ERROR)
            << R"(Model: /backbone/tsm must be "@file@..." or "@binary@...")"
            << orz::crash;
        }
        std::string path = root.empty() ?
                             commands [2] :
                             orz::Join(
                               { root, commands [2] }, orz::FileSeparator());
    ...
    struct ModelParam {
        ...
        static bool to_bool(const orz::jug &jug) { return jug.to_bool(); }
        ...
        static std::vector<std::vector<int>> to_int_list_list(
          const orz::jug &jug) {
          if (jug.invalid(orz::Piece::LIST))
            throw orz::Exception("jug must be list");
        ...
        static std::vector<std::vector<float>> to_float_list_list(
          const orz::jug &jug) {
          if (jug.invalid(orz::Piece::LIST))
            throw orz::Exception("jug must be list");
    ...
    ModelParam parse_model(const orz::jug &model) {
      ...
      if (pre_processor.valid()) {
        if (pre_processor.valid(orz::Piece::LIST)) {
          ...
          ORZ_LOG(orz::ERROR)
            << "Model: /pre_processor must be list" << orz::crash;
        }
      }
      ...
      if (prior_box.valid(orz::Piece::DICT)) {
        try {
          param.prior_box.variance = ModelParam::to_float_list(
            prior_box ["variance"]);
        } catch (const orz::Exception &) {
          ORZ_LOG(orz::ERROR)
            << "Model: /prior_box/variance must be list of float" << orz::crash;
        }
        try {
          param.prior_box.clip = ModelParam::to_bool(prior_box ["clip"]);
        } catch (const orz::Exception &) {
          ORZ_LOG(orz::ERROR)
            << "Model: /prior_box/clip must be boolean" << orz::crash;
        }
        try {
          param.prior_box.steps = ModelParam::to_int_list(prior_box ["steps"]);
        } catch (const orz::Exception &) {
          ORZ_LOG(orz::ERROR)
            << "Model: /prior_box/steps must be list of int" << orz::crash;
        }
        try {
          param.prior_box.aspect_ratios = ModelParam::to_float_list_list(
            prior_box ["aspect_ratios"]);
        } catch (const orz::Exception &) {
          ORZ_LOG(orz::ERROR)
            << "Model: /prior_box/aspect_ratios must be list of float list"
            << orz::crash;
        }
        try {
          param.prior_box.min_sizes = ModelParam::to_int_list_list(
            prior_box ["min_sizes"]);
        } catch (const orz::Exception &) {
          ORZ_LOG(orz::ERROR)
            << "Model: /prior_box/min_sizes must be list of int list"
            << orz::crash;
        }
      } else {
      ...
      if (nms.valid(orz::Piece::DICT)) {
        ...
        if (param.nms.threshold < 0) {
          ORZ_LOG(orz::ERROR)
            << "Model: /nms/threshold must be float in [0, 1]" << orz::crash;
        }
        param.nms.top_k = orz::jug_get<int>(nms ["top_k"], param.nms.top_k);
        param.nms.keep_top_k = orz::jug_get<int>(
          nms ["keep_top_k"], param.nms.keep_top_k);
      } else {
      ...
      if (global.valid(orz::Piece::DICT)) {
        ...
        if (param.global.threshold < 0) {
          ORZ_LOG(orz::ERROR)
            << "Model: /global/threshold must be float in [0, 1]" << orz::crash;
        }
      } else {
    ...
    Device to_ts_device(const seeta::ModelSetting &setting) {
      switch (setting.get_device()) {
        case seeta::ModelSetting::Device::AUTO: return Device("cpu");
        case seeta::ModelSetting::Device::CPU: return Device("cpu");
        case seeta::ModelSetting::Device::GPU: return Device("gpu", setting.id);
        default: return Device("cpu");
      }
    }

    static void build_filter(
      ImageFilter &filter, const std::vector<orz::jug> &pre_processor) {
      for (size_t i = 0; i < pre_processor.size(); ++i) {
        auto &processor = pre_processor [i];
        if (processor.invalid(orz::Piece::DICT)) {
          ORZ_LOG(orz::ERROR) << "Model: the " << i << "-th processor \""
                              << processor << "\" should be dict" << orz::crash;
        }
        auto op = orz::jug_get<std::string>(processor ["op"], "");
        if (op.empty()) {
          ORZ_LOG(orz::ERROR)
            << R"(Model: processor should be set like {"op": "to_float"}.)"
            << orz::crash;
        }
        if (op == "to_float") {
          filter.to_float();
          ...
          ORZ_LOG(orz::ERROR) << "Model: processor \"" << processor
                              << "\" not supported." << orz::crash;
        }
      }
    }
      ...
      public:
        ...
        class Iterator {
          public:
            ...
            Iterator(int M, int N, int m, int n) : M(M), N(N), m(m), n(n) {}

            std::array<int, 2> operator*() const { return { m, n }; }
        ...
        Iterator begin() const { return Iterator(M, N, 0, 0); }

        Iterator end() const { return Iterator(M, N, M, 0); }
      private:
        int M;
        int N;
    };
    ...
    static inline BindingBox decode_single(
      ...
      public:
        ...
        double get(FaceDetector::Property property) const {
          switch (property) {
            ...
            case FaceDetector::PROPERTY_MIN_FACE_SIZE: return m_min_face_size;
            case FaceDetector::PROPERTY_MAX_IMAGE_WIDTH:
              return m_max_image_width;
            case FaceDetector::PROPERTY_MAX_IMAGE_HEIGHT:
              return m_max_image_height;
            case FaceDetector::PROPERTY_NUMBER_THREADS: return m_number_threads;
            case FaceDetector::PROPERTY_ARM_CPU_MODE: return get_cpu_affinity();
          }
        }

        int get_cpu_affinity() const { return m_cpu_affinity; }

        void set_cpu_affinity(int level) {
          switch (level) {
            case 0: m_bench.set_cpu_mode(CpuPowerMode::BIG_CORE); break;
            case 1: m_bench.set_cpu_mode(CpuPowerMode::LITTLE_CORE); break;
            case 2: m_bench.set_cpu_mode(CpuPowerMode::BALANCE); break;
    ...
    FaceDetector::Implement::Implement(const seeta::ModelSetting &setting) {
      ...
      if (param.prior_box.variance.size() != 2) {
        ORZ_LOG(orz::ERROR)
          << "Model: /prior_box/variance size must be 2" << orz::crash;
      }
    ...
    float IoU(const BindingBox &w1, const BindingBox &w2) {
      auto xOverlap = std::max<float>(
        0, std::min(w1.x2 - 1, w2.x2 - 1) - std::max(w1.x1, w2.x1) + 1);
      auto yOverlap = std::max<float>(
        0, std::min(w1.y2 - 1, w2.y2 - 1) - std::max(w1.y1, w2.y1) + 1);
    ...
    std::vector<BindingBox> NMS_sorted(
      std::vector<BindingBox> &winList, float threshold) {
    ...
    SeetaFaceInfoArray FaceDetector::Implement::detect(
      ...
      // get faces
      auto tensor = tensor::build(
        UINT8, { 1, image.height, image.width, image.channels }, image.data);
      ...
      auto priors = prior_box_forward(
        detection_dimension, { input_height, input_width }, location.size(0));
      auto boxes = decode(
        location,
        priors,
        m_param.prior_box.variance,
        confidence,
        m_param.global.threshold,
        m_param.nms.top_k);
    ...
    std::vector<PriorBox> FaceDetector::Implement::prior_box_forward(
      const Tensor             &box_dimension,
      const std::array<int, 2> &image_size,
      int                       tips) const {
          ...
          for (auto &min_size: min_sizes) {
            ...
            if (min_size == 32) {
              ...
              std::vector<float> dense_cx = {
                fj + 0.0f, fj + 0.25f, fj + 0.5f, fj + 0.75f
              };
              std::vector<float> dense_cy = {
                fi + 0.0f, fi + 0.25f, fi + 0.5f, fi + 0.75f
              };
              std::for_each(dense_cx.begin(), dense_cx.end(), [&](float &x) {
                x = x * steps [k] / image_size [1];
              });
              std::for_each(dense_cy.begin(), dense_cy.end(), [&](float &y) {
                y = y * steps [k] / image_size [0];
              });
      ...
      if (m_param.prior_box.clip) {
        ...
        std::for_each(
          begin, end, [&](float &f) { f = f > 1 ? 1 : (f < 0 ? 0 : f); });
      }

      return mean;
    }

    std::vector<BindingBox> FaceDetector::Implement::decode(
      const Tensor                &location,
      const std::vector<PriorBox> &priors,
      const std::vector<float>    &variance,
      const Tensor                &confidence,
      ...
      if (size_t(N) != priors.size()) {
        ORZ_LOG(orz::ERROR)
          << "Internal error with location and priors mismatch (" << N
          << " vs. " << priors.size() << ")";
        return boxes;
      }
      ...
      for (int i = 0; i < N; ++i) {
        ...
        if (boxes.size() < cast_top_k) {
          boxes.insert(
            boxes.end(),
            decode_single(location_data, priors_data, variance_data, score));
          ...
          boxes.back() = decode_single(
            location_data, priors_data, variance_data, score);
          std::push_heap(boxes.begin(), boxes.end(), heap_comparer);
        }
      }
